...
Matrix alloc_matrix(int rows, int cols) {
  ...
  m.rows = rows;
  m.cols = cols;
  m.data = (double_t**) malloc(sizeof(double_t*) * m.rows);
  for (i = 0; i < m.rows; ++i) {
    m.data[i] = (double_t*) malloc(sizeof(double_t) * m.cols);
    for (j = 0; j < m.cols; ++j) {
      m.data[i][j] = 0.0;
    }
  }
...
int equal_matrix(Matrix a, Matrix b, double_t tolerance) {
  uint8_t i,j ;
  for (i = 0; i < a.rows; ++i) {
    for (j = 0; j < a.cols; ++j) {
...
void scale_matrix(Matrix m, double_t scalar) {
  uint8_t i, j;
  for (i = 0; i < m.rows; ++i) {
    for (j = 0; j < m.cols; ++j) {
      m.data[i][j] *= scalar;
...
void swap_rows(Matrix m, int r1, int r2) {
  double_t* tmp = m.data[r1];
  m.data[r1] = m.data[r2];
  m.data[r2] = tmp;
}

void scale_row(Matrix m, int r, double_t scalar) {
  uint8_t i;
  for (i = 0; i < m.cols; ++i) {
    m.data[r][i] *= scalar;
  }
...
void shear_row(Matrix m, int r1, int r2, double_t scalar) {
  uint8_t i;
  for (i = 0; i < m.cols; ++i) {
...
int destructive_invert_matrix(Matrix input, Matrix output) {
  ...
  for (i = 0; i < input.rows; ++i) {
    ...
    double_t scalar = 1.0 / input.data[i][i];
    ...
    continue;
      ...
      double_t shear_needed = -input.data[j][i];
